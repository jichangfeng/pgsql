<?php
namespace Workerman\PostgreSQL;
 ...
 * 在 https://github.com/walkor/mysql 的基础上修改而成
 */
class Connection
{
    /**
     ...
     */
    public function __construct($host, $port, $user, $password, $db_name, $db_schema, $charset = 'utf8')
    {
        $this->settings = array(
            'host'     => $host,
            'port'     => $port,
    ...
    {
        $dsn       = 'pgsql:dbname=' . $this->settings["dbname"] . ';host=' .
            $this->settings["host"] . ';port=' . $this->settings['port'];
        if(!empty($this->settings["user"]))
        {
            $dsn .= ';user='.$this->settings["user"];
            empty($this->settings["user"]) OR $dsn .= ';password='.$this->settings["password"];
        }
        $this->pdo = new PDO($dsn, $this->settings["user"], $this->settings["password"]);
        $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $this->pdo->setAttribute(PDO::ATTR_STRINGIFY_FETCHES, false);
        $this->pdo->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
        if (is_object($this->pdo) && ! empty($this->settings["dbschema"]))
        {
                $this->pdo->query('SET search_path TO '.$this->settings["dbschema"].',public');
        }
    }
